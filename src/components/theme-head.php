...
/**
 * Font link for plced in header
 * 
 * This code is used when need to change font
 * Both fonts were added from google font which vazir is used for persian font and quicksand is used for english font
 */ 
echo '<link href="https://fonts.googleapis.com/css2?family=Quicksand&family=Vazirmatn:wght@400;500&display=swap" rel="stylesheet">';
...
if (! function_exists('mfn_fonts_selected')) {
	...
	{
		$fonts = array(
			'content' => mfn_opts_get('font-content', 'iranyekan'),
			'menu' => mfn_opts_get('font-menu', 'iranyekan'),
			'title' => mfn_opts_get('font-title', 'iranyekan'),
			'headings' => mfn_opts_get('font-headings', 'iranyekan'),
			'headingsSmall' => mfn_opts_get('font-headings-small', 'iranyekan'),
			'blockquote' => mfn_opts_get('font-blockquote', 'iranyekan'),
			'decorative' => mfn_opts_get('font-decorative', 'iranyekan'),
		);
...
if (! function_exists('mfn_styles')) {
	function mfn_styles()
	{
		...
		wp_enqueue_style('mfn-be', get_theme_file_uri('/css/be.css'), false, MFN_THEME_VERSION);
		...
		if ( ! isset( $theme_disable[ 'font-awesome' ] ) ) {
			wp_enqueue_style('mfn-font-awesome', get_theme_file_uri('/fonts/fontawesome/fontawesome.css'), false, MFN_THEME_VERSION);
		}
		...
		if ($google_array) {
			...
			wp_enqueue_style('mfn-fonts', 'https://fonts.googleapis.com/css?family='. $google_array . $subset . '&display=swap');
		}

		// button custom | font

		if( 'custom' == mfn_opts_get( 'button-style' ) ){
			...
			if ( in_array( $button_font_family, $google_fonts ) ) {
				...
				wp_enqueue_style('mfn-font-button', 'https://fonts.googleapis.com/css?family='. $button_font_family . ':400,' . $button_font['weight_style'] . $subset . '&display=swap');
			}
